...
import 'package:smartsystemforschools/features/chatbot/presentation/chat_history_screen.dart';
import 'package:smartsystemforschools/features/main_screen/presentation/views/main_screen.dart';
import 'package:smartsystemforschools/features/settings_view/presentation/manager/themeMode/theme_mode_cubit.dart';

class ChatbotScreen extends StatefulWidget {
  final ChatHistory? chatHistory;
  const ChatbotScreen({super.key, this.chatHistory});
...
class _ChatbotScreenState extends State<ChatbotScreen> {
  ...
  // New variables for chat history
  ...
  void _initChatHistory() {
    if (widget.chatHistory != null) {
      // We're opening an existing chat
      _currentChatId = widget.chatHistory!.id;
      _chatTitle = widget.chatHistory!.title;
      _titleController.text = _chatTitle;

      // Load messages into the chat cubit
      WidgetsBinding.instance.addPostFrameCallback((_) {
        context.read<ChatCubit>().loadMessages(widget.chatHistory!.messages);
      });
    }
  }
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          titleTextStyle: AppStyles.styleSemiBold20(),
          centerTitle: true,
          ...
          actions: [
              ...
              Expanded(
                child: Padding(
                  ...
                  child: TextField(
                    ...
                    onSubmitted: (value) {
                      setState(() {
                        _chatTitle = value.isNotEmpty ? value : 'New Chat';
                        _titleEditing = false;
                      });
                    },
                  ),
            ...
            IconButton(
              icon: Icon(
                _titleEditing ? Icons.check : Icons.edit,
                color: Colors.white,
              ),
              onPressed: () {
                setState(() {
                  if (_titleEditing) {
                    _chatTitle = _titleController.text.isNotEmpty
                        ? _titleController.text
                        : 'New Chat';
                  } else {
                    _titleController.text = _chatTitle;
                  }
                  _titleEditing = !_titleEditing;
                });
              },
            ),
            IconButton(
  ...
  // Methods to handle navigation with saving
  ...
  Future<void> _saveCurrentChat() async {
    final messages = context.read<ChatCubit>().state.messages;
    if (messages.isNotEmpty) {
      if (_currentChatId == null) {
        // Create new chat history
        final chatHistory = await context
            .read<ChatHistoryCubit>()
            .createChatHistory(_chatTitle, messages);
        if (chatHistory != null) {
        ...
        await service.updateChatHistory(
          ChatHistory(
            id: _currentChatId!,
            title: _chatTitle,
  ...
  void _saveAndNavigateToHistory() async {
    await _saveCurrentChat();
    if (mounted) {
  ...
  // Also update your sendMessage method to automatically save after sending
  void _sendMessage(BuildContext context) {
    if (_messageController.text.isNotEmpty ||
        _selectedImagePath != null ||
        _selectedFilePath != null) {
      ...
      // Save the chat after a brief delay to include the AI response
